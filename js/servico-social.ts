    ...
    async function carregarTodasSolicitacoesDoFirestore() {
        ...
        try {
            ...
            if (resultado.sucesso) {
                ...
                throw new Error(resultado.codigoErro || resultado.mensagemErro || "Falha ao buscar solicitações.");
            }
        } catch (error) {
            ...
            console.error("ERRO FINAL DO FIREBASE:", error); // Loga o erro completo no console
            if (error.code && error.code === 'failed-precondition') {
                mostrarErro(solicitacoesPreAprovadasContainer, `Erro de Índice do Firestore: Sua consulta requer um índice. Verifique o console do Firebase para criar o índice necessário.`);
                mostrarErro(historicoValidacoesContainer, `Erro de Índice do Firestore.`);
                return;
            }
